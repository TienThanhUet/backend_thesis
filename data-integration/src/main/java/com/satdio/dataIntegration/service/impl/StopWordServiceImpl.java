    ...
    public void removeStopWord() {
        ...
        while (page<=pageQuantity){
            ...
            movieList.stream().forEach(movie -> {
                String storyLine = movie.getStoryline().toLowerCase();
                String[] wordList = storyLine.split("\\s+");

                StringBuilder storyLineStopWord = new StringBuilder();
                for(int pos = 0;pos<wordList.length;pos++){
                    int have = stopWordsList.indexOf(wordList[pos]);
                    if(have!=-1) storyLineStopWord.append(wordList[pos]);
                }

                String tconst = movie.getTconst();
                MovieStopWord movieStopWord = movieStopWordRepository.findFirstByTconst(tconst);
                if(movieStopWord==null){
                    movieStopWord = new MovieStopWord();
                }
                movieStopWord.setTconst(tconst);
                movieStopWord.setTitle(movie.getPrimaryTitle());
                movieStopWord.setGenres(movie.getGenres());
                movieStopWord.setStoryStandard(storyLineStopWord.toString());
